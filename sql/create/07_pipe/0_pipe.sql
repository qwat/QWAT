...
ALTER TABLE distribution.pipe ADD COLUMN id_function            integer not null;                             /* id_function          FK */ 
ALTER TABLE distribution.pipe ADD COLUMN id_installmethod       integer not null;                             /* id_installmethod     FK */
ALTER TABLE distribution.pipe ADD COLUMN id_material            integer not null;                             /* id_material          FK */
ALTER TABLE distribution.pipe ADD COLUMN id_distributor         integer not null;                             /* id_distributor       FK */
ALTER TABLE distribution.pipe ADD COLUMN id_precision           integer not null;                             /* id_precision         FK */
ALTER TABLE distribution.pipe ADD COLUMN id_protection          integer not null;                             /* id_protection        FK */
ALTER TABLE distribution.pipe ADD COLUMN id_status              integer not null;                             /* id_status            FK */
ALTER TABLE distribution.pipe ADD COLUMN id_watertype           integer not null;                             /* id_watertype         FK */
ALTER TABLE distribution.pipe ADD COLUMN id_label_visible       integer default 2;                            /* label_view           FK */
...
ALTER TABLE distribution.pipe ADD CONSTRAINT pipe_id_label_visible       FOREIGN KEY (id_label_visible)       REFERENCES distribution.vl_visible(vl_code) MATCH FULL   ; CREATE INDEX fki_pipe_id_label_visible       ON distribution.pipe(id_label_visible);
ALTER TABLE distribution.pipe ADD CONSTRAINT pipe_id_labelschema_visible FOREIGN KEY (id_labelschema_visible) REFERENCES distribution.vl_visible(vl_code) MATCH FULL   ; CREATE INDEX fki_pipe_id_labelschema_visible ON distribution.pipe(id_labelschema_visible);
