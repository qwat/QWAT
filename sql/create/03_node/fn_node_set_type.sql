...
CREATE OR REPLACE FUNCTION qwat.fn_node_set_type_all(extent box2d DEFAULT NULL, offset_prog integer DEFAULT NULL, limit_prog integer DEFAULT NULL) RETURNS void AS
$BODY$
  DECLARE
    node record;
  BEGIN
    ...
    ELSE
      FOR node IN (
        SELECT id
        FROM qwat.od_node
        WHERE qwat.od_node.geometry && extent
        ORDER BY id
        LIMIT limit_prog
        OFFSET offset_prog
      )
