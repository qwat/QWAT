...
CREATE OR REPLACE FUNCTION qwat.fn_geom_tool_line(table_name varchar) RETURNS void AS
$BODY$
	BEGIN
		/* Creates columns */
		EXECUTE 'ALTER TABLE qwat.'||table_name||' ADD COLUMN id_node_a       integer    not null;';
		EXECUTE 'ALTER TABLE qwat.'||table_name||' ADD COLUMN id_node_b       integer    not null;';
		EXECUTE 'ALTER TABLE qwat.'||table_name||' ADD COLUMN id_district     integer   ;';
		EXECUTE 'ALTER TABLE qwat.'||table_name||' ADD COLUMN id_pressurezone integer   ;';
		EXECUTE 'ALTER TABLE qwat.'||table_name||' ADD COLUMN id_printmap     integer[] ;';
		EXECUTE 'ALTER TABLE qwat.'||table_name||' ADD COLUMN _length2d       decimal(8,2) ;';
		EXECUTE 'ALTER TABLE qwat.'||table_name||' ADD COLUMN _length3d       decimal(8,2) ;';	
		EXECUTE 'ALTER TABLE qwat.'||table_name||' ADD COLUMN _diff_elevation decimal(8,2) ;';	
		EXECUTE 'ALTER TABLE qwat.'||table_name||' ADD COLUMN _district       varchar(255) default '''' ;';
		...
		EXECUTE 'ALTER TABLE qwat.'||table_name||' ADD CONSTRAINT '||table_name||'_id_node_b       FOREIGN KEY (id_node_b)       REFERENCES qwat.od_node(id)         MATCH FULL;';
		EXECUTE 'ALTER TABLE qwat.'||table_name||' ADD CONSTRAINT '||table_name||'_id_district     FOREIGN KEY (id_district)     REFERENCES qwat.od_district(id)     MATCH SIMPLE;';
		EXECUTE 'ALTER TABLE qwat.'||table_name||' ADD CONSTRAINT '||table_name||'_id_pressurezone FOREIGN KEY (id_pressurezone) REFERENCES qwat.od_pressurezone(id) MATCH SIMPLE;';
		EXECUTE 'CREATE INDEX fki_'||table_name||'_id_node_a       ON qwat.'||table_name||'(id_node_a);';
		EXECUTE 'CREATE INDEX fki_'||table_name||'_id_node_b       ON qwat.'||table_name||'(id_node_b);';
		EXECUTE 'CREATE INDEX fki_'||table_name||'_id_district     ON qwat.'||table_name||'(id_district);';
		EXECUTE 'CREATE INDEX fki_'||table_name||'_id_pressurezone ON qwat.'||table_name||'(id_pressurezone);';
		
		/* Geometric trigger function */
		EXECUTE '
			CREATE OR REPLACE FUNCTION qwat.'||table_name||'_geom() RETURNS TRIGGER AS
				''
				BEGIN
...
$BODY$
LANGUAGE 'plpgsql';
COMMENT ON FUNCTION qwat.fn_geom_tool_line(varchar) IS 'Create geometric columns, constraint and triggers for tables with line items.';
