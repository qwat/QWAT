...
CREATE OR REPLACE FUNCTION qwat.od_dimension_distance_distance() RETURNS trigger AS 
$BODY$
	BEGIN
...
CREATE TRIGGER dimension_distance_trigger 
	...
	FOR EACH ROW
	EXECUTE PROCEDURE qwat.od_dimension_distance_distance();
COMMENT ON TRIGGER dimension_distance_trigger ON qwat.od_dimension_distance IS 'Trigger: updates the length and other fields of the pipe after insert/update.';
