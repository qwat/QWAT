...
/* constraints */
ALTER TABLE distribution.leak ADD CONSTRAINT leak_id_type   FOREIGN KEY (id_type)   REFERENCES distribution.leak_type(id)   MATCH FULL;   CREATE INDEX fki_leak_id_type   ON distribution.leak(id_type)  ;
ALTER TABLE distribution.leak ADD CONSTRAINT leak_id_cause  FOREIGN KEY (id_cause)  REFERENCES distribution.leak_cause(id)  MATCH SIMPLE; CREATE INDEX fki_leak_id_cause  ON distribution.leak(id_cause) ;
ALTER TABLE distribution.leak ADD CONSTRAINT leak_id_damage FOREIGN KEY (id_damage) REFERENCES distribution.leak_damage(id) MATCH FULL;   CREATE INDEX fki_leak_id_damage ON distribution.leak(id_damage);
