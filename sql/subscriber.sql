...
/*----------------!!!---!!!----------------*/
/* INSERT without id_district => get district */
CREATE OR REPLACE FUNCTION distribution.subscriber_insert() RETURNS trigger AS ' 
	...
	END;
' LANGUAGE 'plpgsql';
COMMENT ON FUNCTION distribution.subscriber_insert() IS 'Fcn/Trigger: updates the disctrict and map of the subscriber after insert/update.';
...
COMMENT ON TRIGGER subscriber_insert_trigger ON distribution.subscriber IS 'Trigger: updates the disctrict and map of the subscriber after insert/update.';
